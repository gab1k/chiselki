...
namespace adaai {
    ...
    void log_info(std::ofstream &ofs, F min_value, F max_value) {
        ...
        ofs << "Absolute error for  x < 0 = " << errs_m.first << "; It is " << errs_m.first / C_EPS<F>
            << " * eps\n";
        ofs << "Relative error for x >= 0 = " << errs_p.second << "; It is " << errs_p.second / C_EPS<F>
            << " * eps\n";
        F our_err = std::max(errs_p.second, errs_m.first);
        ofs << "Our error is              = " << our_err << "; It is " << our_err / C_EPS<F> << " * eps\n\n";

    }

    TEST_CASE("Log Error Float") {
        std::string filePath = "../exp/logs/float_logs.txt";
    ...
    TEST_CASE("Log Error Double") {
        std::string filePath = "../exp/logs/double_logs.txt";
        std::ofstream ofs(filePath.c_str(), std::ios_base::out);
        ofs << "Eps = " << C_EPS<double> << "\n\n";
        ofs << "Taylor:\n";
    ...
    TEST_CASE("Log Error Long Double") {
        std::string filePath = "../exp/logs/long_double_logs.txt";
        std::ofstream ofs(filePath.c_str(), std::ios_base::out);
        ofs << "Eps = " << C_EPS<long double> << "\n\n";
        ofs << "Taylor:\n";
